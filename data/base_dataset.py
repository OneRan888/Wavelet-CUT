...
class BaseDataset(data.Dataset, ABC):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.add_argument('--overlap_ratio', type=float, default=0.25,
                            help='Overlap ratio between adjacent patches')
        return parser
...
def get_params(opt, size):
    ...
    # 处理重叠切片的位置参数
    if 'overlap_patch' in opt.preprocess:
        patch_size = opt.crop_size
        overlap = int(patch_size * opt.overlap_ratio)
        step = patch_size - overlap

        # 计算有效切片区域
        valid_w = (w // step) * step - (patch_size - step)
        valid_h = (h // step) * step - (patch_size - step)

        # 确保有效区域至少能容纳一个patch
        valid_w = max(valid_w, patch_size)
        valid_h = max(valid_h, patch_size)

        # 随机选择切片起始位置
        x = random.randint(0, valid_w - patch_size) if valid_w > patch_size else 0
        y = random.randint(0, valid_h - patch_size) if valid_h > patch_size else 0
        params['overlap_patch_pos'] = (x, y)
...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    # 添加重叠切片处理逻辑
    if 'overlap_patch' in opt.preprocess:
        if params is None or 'overlap_patch_pos' not in params:
            # 若未指定参数，默认从左上角截取
            transform_list.append(transforms.Lambda(
                lambda img: __overlap_patch(img, (0, 0), opt.crop_size)))
        else:
            transform_list.append(transforms.Lambda(
                lambda img: __overlap_patch(img, params['overlap_patch_pos'], opt.crop_size)))
    ...
    if 'trim' in opt.preprocess:
        transform_list.append(transforms.Lambda(
            lambda img: __trim(img, opt.crop_size)))
...
def __overlap_patch(img, pos, size):
    """生成带重叠的固定尺寸切片"""
    ow, oh = img.size
    x1, y1 = pos
    tw, th = size, size
    ...
    if x2 - x1 < tw or y2 - y1 < th:
        ...
        # 创建空白画布
        new_img = Image.new(img.mode, (tw, th), color=0)
