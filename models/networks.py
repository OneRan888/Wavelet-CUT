...
# ---------------------- 可选导入改造（核心修改：容错StyleGAN2缺失）----------------------
# 尝试导入StyleGAN2相关类，缺失则设为None，不影响基础功能
try:
    from .stylegan_networks import StyleGAN2Discriminator, StyleGAN2Generator, TileStyleGAN2Discriminator
except ModuleNotFoundError:
    # 缺失StyleGAN2文件时，相关类设为None，后续做容错判断
    StyleGAN2Discriminator = None
    StyleGAN2Generator = None
    TileStyleGAN2Discriminator = None
# ----------------------------------------------------------------------------------------
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    Parameters:
        ...
        init_gain (float)  -- scaling factor for normal, xavier and orthogonal.
        gpu_ids (int list) -- that GPUs the network runs on: e.g., 0,1,2
    ...
    if netG == 'resnet_9blocks':
        ...
        # ---------------------- 容错改造：判断StyleGAN2类是否存在 ----------------------
        if StyleGAN2Generator is None:
            raise NotImplementedError('Generator model name [stylegan2] is not supported: missing stylegan_networks.py file (you do not need this for basic CUT mode).')
        net = StyleGAN2Generator(input_nc, output_nc, ngf, use_dropout=use_dropout, opt=opt)
    elif netG == 'smallstylegan2':
        # ---------------------- 容错改造：判断StyleGAN2类是否存在 ----------------------
        if StyleGAN2Generator is None:
            raise NotImplementedError('Generator model name [smallstylegan2] is not supported: missing stylegan_networks.py file (you do not need this for basic CUT mode).')
        net = StyleGAN2Generator(input_nc, output_nc, ngf, use_dropout=use_dropout, n_blocks=4, opt=opt)
        n_blocks = 8
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        # ---------------------- 容错改造：判断StyleGAN2类是否存在 ----------------------
        if StyleGAN2Discriminator is None or TileStyleGAN2Discriminator is None:
            raise NotImplementedError('Discriminator model name [%s] is not supported: missing stylegan_networks.py file (you do not need this for basic CUT mode).' % netD)
...
def cal_gradient_penalty(netD, real_data, fake_data, device, type='mixed', constant=1.0, lambda_gp=10.0):
    ...
    Arguments:
        ...
        constant (float)            -- constant for gradient penalty
